window.addEventListener('load',function(){
    btn1.addEventListener('click',function(){
        ...
        console.log(regExp1,typeof(regExp1),typeof(regExp1),regExp1 instanceof RegExp);

        console.log(regExp2,typeof(regExp2),typeof(regExp2),regExp2 instanceof RegExp);
        ...
        area1.innerHTML
                += `regExp1.test(str2): ${regExp1.test(str2)} <br>`;
        //////////////////////////////////////////////////////////////////
        area1.innerHTML
                += `regExp2.exec(str1): ${regExp2.exec(str1)} <br>`;
        area1.innerHTML
                += `regExp2.exec(str2): ${regExp2.exec(str2)} <br><br>`;

        /*
         String 객채에서 정규 표현식 객체를 이용하는 메소드
            문자열.match(정규식)
        ...
        area1.innerHTML += 'str1.serach(regExp1) ==> ' + str1.search(regExp1) + '<br>'
    ...
    btn3.addEventListener('click',function(){
        ...
        area3.innerHTML = '<h3>메타 문자</h3>';


        /*
        반복 검색 
        {n,m}
            앞선 패턴이 최소 n번 최대m번 반복되는 문자열을 의미한다.
        +
        ...
        */
        ...
        //{3,3}을 뜻함
        area3.innerHTML+= '/a{3}/g ==> '+ str.replace(/a{3}/g,'($&)') + '<br>'
        //앞선 패턴이 최소 2번 이상 반복되는 문자열
        area3.innerHTML+= '/a{2,}/g ==> '+ str.replace(/a{2,}/g,'($&)') + '<br>'
        //{1,}
        area3.innerHTML+= '/a+/g ==> '+ str.replace(/a+/g,'($&)') + '<br>'
        //{0,1}
        area3.innerHTML+= '/a?/g ==> '+ str.replace(/a?/g,'($&)') + '<br>'
        ...
        문자열의 앞, 뒤 구분
            ...
            $는 문자열의 마지막을 의미함.
        */
        area3.innerHTML+= 'str1.replace(/^j/ig ==> ' + str1.replace(/^j/ig,'($&)')+ '<br>';
        area3.innerHTML+= 'str1.replace(/^j/igm ==> ' + str1.replace(/^j/igm,'($&)' )+ '<br>';
        area3.innerHTML+= 'str1.replace(/ipt$/igm ==> ' + str1.replace(/ipt$/igm,'($&)' )+ '<br>';
        ...
        area3.innerHTML+= '/[^aj]/ig ==> ' + str.replace(/[^aj]/ig, '($&)')+ '<br>';
        area3.innerHTML+= '/[a-z]/g ==> ' + str.replace(/[a-z]/g, '($&)')+ '<br>';
        area3.innerHTML+= '/[A-Z]/g ==> ' + str.replace(/[A-Z]/g, '($&)')+ '<br>';
        area3.innerHTML+= '/[0-9]/ig ==> ' + str.replace(/[0-9]/ig, '($&)')+ '<br><br>';
        ...
        */
        ...
        \d  : 숫자를 의미한다. == [0-9]
        \D  : 숫자가 아닌 문자를 의미 == [^0-9]
        \w  : 알파벳,숫자,언더 스코어(_)  를 의미 == [a-zA-Z0-9] 
        \W  : 알파벳,숫자,언더스코어가 아닌 문자를 의미 == [^a-zA-Z0-9]
        \s  : 공백 문자를 의믜 (띄어쓰기 ,탭 ,줄바꿈) == [\n\t]
        \S  : 공백 문자가 아닌 문자를 의미 == [^ \n\t]
        */
    })
